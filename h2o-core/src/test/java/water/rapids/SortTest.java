...
public class SortTest extends TestUtil {
  ...
  @Test public void TestSortIntegersFloats() throws IOException {
    ...
    TestSortOneColumn("smalldata/synthetic/smallIntFloats.csv.zip", 0, false);
    // test small float sort
    TestSortOneColumn("smalldata/synthetic/smallIntFloats.csv.zip", 1, false);
    // test integer frame
    TestSortOneColumn("smalldata/synthetic/integerFrame.csv", 0, false);
    // test integer frame with NAs
    TestSortOneColumn("smalldata/synthetic/integerFrame.csv", 0, true);
    // test double frame
    TestSortOneColumn("smalldata/synthetic/doubleFrame.csv", 0, false);
    // test double frame with NAs
    TestSortOneColumn("smalldata/synthetic/doubleFrame.csv", 0, true);
    // test integer frame where overflow will occur for col.max()-col.min()
    TestSortOneColumn("smalldata/synthetic/bigIntFloatsOverflows.csv.zip", 0, false);
    // test integer frame where overflow will occur for col.max()-col.min(), with NAs
    TestSortOneColumn("smalldata/synthetic/bigIntFloatsOverflows.csv.zip", 0, true);
    // test double frame where overflow will occur for col.max()-col.min()
    TestSortOneColumn("smalldata/synthetic/bigIntFloatsOverflows.csv.zip", 1, false);
    // test double frame where overflow will occur for col.max()-col.min(), with NAs
    TestSortOneColumn("smalldata/synthetic/bigIntFloatsOverflows.csv.zip", 1, true);
  }

  /*
  Test sorting of integers and floats of small magnitude, 2^30 and no NANs or INFs
 */
  private static void TestSortOneColumn(String fileWithPath, int colIndex, boolean addNas) throws IOException {
    Scope.enter();
